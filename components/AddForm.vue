...
<div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
    ...
    <div class=" w-full mx-8 bg-white overflow-hidden shadow sm:rounded-lg p-6 mx-auto ">
       <h1>BUAT BUKU CATATAN</h1>
      <div class="m-9">

        <form @submit.prevent="submitForm">
          <div class="w-full mb-4">
            <label class="block">Title:</label>
            <input type="text" v-model="title" class="w-full bg-gray-100 text-gray-800 border border-gray-300 px-4 py-2 rounded-md" />
          </div>
          <div class="w-full mb-4">
            <label class="block">Content:</label>
            <textarea v-model="content"  ref="textarea"
      @input="autoResize" class="w-full bg-gray-100 text-gray-800 border border-gray-300 px-4 py-2 rounded-md"></textarea>
          </div>
          <div class="w-full mb-4">

<b-button variant="warning" type="submit" size="lg">Submit</b-button>
          </div>
        </form>
      </div>
    </div>
  ...
  methods: {
    ...
    async submitForm() {
      try {
        ...
        const config = {
          headers: { Authorization: `Basic YWRtaW46VWVPQSBOeE5qIDZXQ3MgemZPSSB4YWVHIEhGUHo=` }
        }

        const response = await axios.post('https://myblog.nufat.id/wp-json/wp/v2/posts', {
          title: this.title,
          content: this.content,
          status:"publish"
        }, config)
// Redirect ke halaman index setelah post berhasil
    this.$router.push('/')
