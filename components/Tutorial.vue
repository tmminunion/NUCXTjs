    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
      ...
      <div class="mt-8 bg-white overflow-hidden shadow sm:rounded-lg p-6">
        ...
        <p class="mt-3 text-gray-600">
          Buku catatan buat nulis catatan apa aja...!!! catatan hutang atau catan diary
         <br />

        </p>
 <div class="d-flex justify-content-center">
   <router-link to="/add">
    <b-button class="m-4" variant="primary">Buat Catatan</b-button></router-link>

  </div>



 <div class="mt-3 border-t border-dashed">
    <p v-if="$fetchState.pending">Loading....</p>
    <p v-else-if="$fetchState.error">Error while fetching mountains</p>
    <ul v-else class="list-group">
      <li v-for="(mountain, index) in mountains" :key="index" class="list-group-item">
        <h1 class="mb-0">
          <nuxt-link :to="'/detail/' + mountain.id" class="text-decoration-none">
            {{ index + 1 }} {{ mountain.title.rendered }}
          </nuxt-link>
        </h1>
      </li>
    </ul>
...
<style>
  /* Add custom styles for list items and links */
  .list-group-item {
    border-radius: 0;
  }
